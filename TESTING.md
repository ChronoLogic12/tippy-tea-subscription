...
- Easily understand the sites purpose and learn more about the services offered.
  - The sites homepage background image and introduction paragraph quickly inform first time users of the sites purpose. This is then further expanded upon if the user scrolls to the 'How it Works' section of the homepage or visits the about us page.
  <p align="center">
...
- Login to my account.
  - Users can navigate to the log in page from the navbar on any page. Entering their details here will log them in.
  <p align="center">
...
- Cancel/amend my current subscription.
  - If the user has any active subscriptions they can be canceled from the manage subscriptions button on the profile page.
...
- Log in to an admin account.
  - Admin users can navigate to the log in page from the navbar on any page. Entering their admin credentials here will log them in.
  <p align="center">
  <img src="readme-assets\sign-in.PNG" width="250px"/>
  </p>
...
To fully test my application in both the development and production environments I devised a series of manual tests to assure all aspect of the site work as intended. By completing these tests in the development and production environments concurrently I can also easy identify any differences between the two.
...
- Check navigation links are displayed as expected and lead to the intended pages when selected.
  ...
  - Logged In: Subscriptions, About Us, Blog, Profile, Log Out.
- Current page is highlighted in navbar with active styling.
- Brand name directs user to homepage when clicked.
...
- Chevron moves user view to 'How it works' section.
...
- Each card contains a select button which take the user to checkout with the correct info when clicked.
...
- Logged in as admin
  ...
  - Add new post button renders at top of page. Clicking this take to add new blog page.

8. Blog (/blog/add/)

- If accessed by non admin user they are redirected to the log in page.
- Page renders a blog form with fields for title, author, content, and imageUrl as well as cancel and add buttons.
- Selecting cancel returns user to the blogs page.
- Form will only submit if valid data has been entered.
- Selecting add will add the new blog post to the database and return the user ot the blogs page and render the new post.
...
- If accessed by a non user they are redirected to the log in page.
...
- If the user selects the delete account button it brings up a confirmation dropdown, and will remove user if confirmed. The user account will then be removed from the database and the user logged out.
...
11. Send Newsletter (/mailing/send-newsletter)

- If accessed by a non admin user they are redirected to the log in page.
...
- Filling in the fields and selecting the send button will send a email made from the submitted data to all emails on the mailing list.
...
- Page render a form containing an email input and a submit button.
- If the user enter a valid email and presses the submit button the entered email will be removed from the mailing list database table.
...
- Renders a form for users to enter username,e-mail, email-confirmation, password, password-confirmation and a sign up button.
- Entering invalid data prompts the user to check their input.
...
16. Confirm Email (/accounts/confirm-email/)
...
- Upon selecting confirm the users email address is confirmed and the user is redirected to the login page.
...
- Upon selecting the sign out button the users is logged out and redirected to the homepage.

19. Checkout (/checkout/)

- If the user is not logged in or navigates to this page outside of the subscriptions page (creates a valid query param and makes post request) they are redirected to the login/homepage.
...
- If valid profile data is enter the profile record associated with the user is updated and they are redirected to the stripe checkout portal.
...
22. Checkout Cancel (/checkout/cancel/)
